...
public class Seguro {
	...
	public String Condutores() {
		String cod = "";
		for(int i = 0;i < listaCondutores.size() - 1;i++) {
			cod += listaCondutores.get(i).getCpf() + "*";
		}
		cod += listaCondutores.get(listaCondutores.size()-1).getCpf();
		return cod;
	}
	...
	public String Sinistros() {
		...
		for(int i = 0;i < listaSinistros.size() - 1;i++) {
			cod += listaSinistros.get(i).getId() + "-";
		}
		cod += listaSinistros.get(listaSinistros.size()-1).getId();
		return cod;
	}
